...
class JukugoJumble {
    ...
    private void windowQuest() {
        ...
        int count = quest[2].length();
        for (int i = 3; i < quest.length; i++) {
            quest[2] += " " + quest[i];
            count += quest[i].length() + 1;
            if (count > 20) {
                count = 0;
                quest[2] += "\r\n";
            }
    ...
    private void hintMe() {
        float hp = live.getValue();
        int hintNum = quest[1].length() - countAnswer;
        if (hp > 0) {
            live.setValue(Math.max(hp - 20 * hintNum / quest[1].length(), 0));
            int hintLoc = (int) (Math.random() * (hintNum)) + 1;
            for (int i = 0; i < quest[1].length(); i++)
                if (listAnswer[i].getText().toString().equals(" ")) {
                    hintLoc--;
                    if (hintLoc == 0) {
                        for (int j = 0; j < 24; j++)
                            if (listChosen[j].isVisible() && listChosen[j].getText().toString().equals(quest[1].charAt(i) + "")) {
                                listAnswer[i].setText(listChosen[j].getText().toString());
                                answerMap[i] = listChosen[j];
                                countAnswer++;
                                listChosen[j].setVisible(false);
                                checkUpdate();
                                break;
                            }
                        break;
    ...
    private void header() {
        ...
        live = createProgressBar(golden, "default-horizontal", header);
        header.getCell(live).size(173.0f, 51.0f).expandX().padLeft(20).left();

        TextButton btnHint = createButton("Hint", glassy, "small", header, new ChangeListener() {
            @Override
            public void changed(ChangeEvent event, Actor actor) {
                hintMe();
            }
        ...
        header.getCell(btnHint).padRight(20);
